...
<div class="row"><div class="container">
    ...
    <form method="post" action="?" class="card" data-parsley-validate>
      ...
      <div class="card-body">
            ...
            <input type="number" name="feed_required_content_length" id="feed_required_content_length" class="form-control" value="<?= sp_post('feed_required_content_length', $t['feed.feed_required_content_length']); ?>" maxlength="10" min="0" required>
            <div class="form-text text-muted">
            ...
            <input type="number" name="feed_content_maxlength" id="feed_content_maxlength" class="form-control" value="<?= sp_post('feed_content_maxlength', $t['feed.feed_content_maxlength']); ?>" maxlength="10" min="0" required>
            <div class="form-text text-muted">
            ...
            <div class="form-group">
              ...
              <select id="feed_keyword_mode" class="form-control" name="feed_keyword_mode">
                <?php foreach ($t['feed_keyword_mode_list'] as $key => $value) : ?>
                  <option value="<?= e_attr($key); ?>" <?= selected($key, sp_post('feed_keyword_mode', $t['feed.feed_keyword_mode'])); ?>><?= e($value); ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            <div class="form-group" id="keyword_group" <?= (int) $t['feed.feed_keyword_mode'] ? '' : 'style="display:none;"'; ?>>
            <label class="form-label" for="feed_required_keywords"><?= __('Keywords'); ?></label>
            <textarea name="feed_required_keywords" id="feed_required_keywords" class="form-control"><?= sp_post('feed_required_keywords', $t['feed.feed_required_keywords']); ?></textarea>
            <div class="form-text text-muted">
        ...
        <section class="fulltext-dom-1">
          ...
          <div class="form-group">
          <button class="btn btn-sm btn-outline-danger" type="button" data-toggle="collapse" data-target="#selector" aria-expanded="<?= empty($t['feed.feed_fulltext_selector']) ? 'false' : 'true'; ?>"><?=svg_icon('settings', 'svg-sm'); ?> <?= __('Advanced'); ?></button>
        </div>

          <div class="form-group collapse <?= empty($t['feed.feed_fulltext_selector']) ? '' : 'show'; ?>" id="selector">
            <label class="form-label" for="feed_fulltext_selector"><?= __('Fulltext CSS Selector (Advanced)'); ?></label>
            <input class="form-control" maxlength="100" type="text" name="feed_fulltext_selector" id="feed_fulltext_selector" placeholder="#content .article-body" value="<?= sp_post('feed_fulltext_selector', $t['feed.feed_fulltext_selector']); ?>">
