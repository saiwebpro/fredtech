...
if (THUMB_BROWSER_CACHE && (isset($_SERVER['HTTP_IF_MODIFIED_SINCE']) || isset($_SERVER['HTTP_IF_NONE_MATCH']))) {
    if (isset($_SERVER['HTTP_IF_MODIFIED_SINCE']) &&  $_SERVER['HTTP_IF_MODIFIED_SINCE'] == $file_date && $_SERVER['HTTP_IF_NONE_MATCH'] == $file_hash) {
...
$app->response->headers->set('Content-Type', 'image/' . $file_type);
$app->response->headers->set('Content-Disposition', "inline; filename=\"{$file_name}\"");
$app->response->headers->set('Content-Length', filesize($file_temp));
$app->response->headers->set('Last-Modified', $file_date);
$app->response->headers->set('ETag', $file_hash);
$app->response->headers->set('Accept-Ranges', 'none');

if (THUMB_BROWSER_CACHE) {
    $app->response->headers->set('Cache-Control', 'max-age=604800, must-revalidate');
    $app->response->headers->set('Expires', gmdate('D, d M Y H:i:s T', strtotime('+7 days')));
} else {
    $app->response->headers->set('Cache-Control', 'no-store, no-cache, must-revalidate, max-age=0');
    $app->response->headers->set('Expires', gmdate('D, d M Y H:i:s T'));
    $app->response->headers->set('Pragma', 'no-cache');
}
$app->response->setBody(file_get_contents($file_temp));
